...
export const getChatByOrderId = async (orderId: number) => {
...
export const getMessagesForOrder = async (orderId: number) => {
  const messages = await prisma.chat_messages.findMany({
    where: { chats: { order_id: orderId } },
    orderBy: { sent_at: 'asc' },
...
export const resetUnreadCount = async (chatId: number, userId: number) => {
...
export const createChat = async (orderId: number, customerId: number, riderId: number) => {
...
export const getOrCreateChat = async (orderId: number) => {
  ...
  if (!chat) {
    try {
      ...
      if (!rider_id) {
        throw {
          status: 400,
          message: 'Cannot start a chat before a rider has accepted the order.',
        };
      }

      chat = await createChat(orderId, user_id!, rider_id!);
    } catch (insertError) {
      if ((insertError as { code?: string }).code === 'P2002') {
...
export const saveMessage = async (chatId: number, userId: number, message: string) => {
...
export const incrementOtherUnreadCounts = async (chatId: number, senderId: number) => {
  await prisma.chat_participants.updateMany({
