...
export const registerRestaurant = async (data) => {
  const { name, email, password, phone, phone_number, latitude, longitude, street, city, postal_code } = data;
...
export const getRestaurantFullProfile = async (restaurantId) => {
  const restaurant = await prisma.restaurants.findUnique({
    where: { restaurant_id: restaurantId },
    include: {
  ...
  const sortedHours = (restaurant.restaurant_hours || []).sort(
    (a, b) => (dayOrder[a.day_of_week] || 8) - (dayOrder[b.day_of_week] || 8)
  );
...
export const updateRestaurantProfile = async (restaurantId, profileData, newImageUrl) => {
  const {
    restaurant_name,
    name,
    phone,
...
export const changeRestaurantPassword = async (restaurantId, prevPassword, newPassword) => {
  const restaurant = await prisma.restaurants.findUnique({
    where: { restaurant_id: restaurantId },
  });
...
export const addMenuItemWithImage = async (restaurantId, itemData, imageUrl) => {
  ...
  const newItem = await prisma.menu_items.create({
    data: {
      ...
      menu_item_image_url: imageUrl || "https://images.unsplash.com/photo-1546793665-c74683f339c1?w=300&h=200&fit=crop",
    },
    include: {
...
export const editMenuItemWithImage = async (restaurantId, menuItemIdRaw, itemData, newImageUrl) => {
  const menuItemId = parseInt(menuItemIdRaw, 10);
  const { name, category, description, price, is_available, discount, menu_item_image_url } = itemData;
  ...
  const finalImageUrl = newImageUrl !== undefined ? newImageUrl : (menu_item_image_url || item.menu_item_image_url);
...
export const changeMenuItemAvailability = async (restaurantId, menuItemIdRaw, status) => {
  const menuItemId = parseInt(menuItemIdRaw, 10);
...
export const deleteMenuItemSoft = async (restaurantId, menuItemIdRaw) => {
  const menuItemId = parseInt(menuItemIdRaw, 10);
