...
export const getNearbyRestaurants = async (userId, page, limit) => {
  const radius = 5; // 5km
  const offset = (page - 1) * limit;

  // Get customer's primary location
  const userLocationResult = await prisma.$queryRaw`
...
export const getRestaurants = async (page, limit) => {
  ...
  const totalResult = await prisma.$queryRaw`SELECT COUNT(*) FROM restaurants`;
...
export const getRestaurant = async (restaurantId) => {
  const restaurantRes = await prisma.$queryRaw`
    SELECT
      r.restaurant_id,
    ...
    WHERE r.restaurant_id = ${parseInt(restaurantId)}
  `;
  ...
  const menuItems = await prisma.$queryRaw`
    ...
    WHERE mc.restaurant_id = ${parseInt(restaurantId)} AND mi.is_active = true
  `;
  ...
  const hoursRes = await prisma.$queryRaw`
  ...
  return {
    ...
    menuItems: menuItems.map(item => ({...item, order_count: Number(item.order_count) || 0})),
  };
};

export const getRestaurantByLocation = async (userLat, userLon, radiusMeters) => {
  const query = async (radius) => prisma.$queryRaw`
    SELECT 
      r.restaurant_id, r.name, r.phone, r.email, r.average_rating, r.image_url,
      ST_Distance(
...
export const getReviewsAll = async (restaurantId) => {
...
export const getRestaurantsSearchByName = async (rest_name, page, limit) => {
  const offset = (page - 1) * limit;
  ...
  const totalResult = await prisma.$queryRaw`SELECT COUNT(*) FROM restaurants r WHERE name ILIKE ${searchPattern}`;
