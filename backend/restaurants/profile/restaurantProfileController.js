  ...
  try {
    ...
    if (status === "ready_for_pickup") {
      ...
      if (deliveryDetailsResult.rows.length > 0) {
        ...
        // Find available riders within 5km radius of the restaurant via Redis
        let availableRiders = [];
        try {
          // Requires Redis integration
          const redisClient = (await import("../../utils/redisClient.js")).default;
          // geoRadius returns an array of members
          const nearbyRiderIds = await redisClient.geoRadius(
            "active_riders",
            { longitude: deliveryDetails.rest_longitude, latitude: deliveryDetails.rest_latitude },
            5,
            "km"
          );

          // Filter out riders whose TTL key has expired
          for (const rId of nearbyRiderIds) {
            const isActive = await redisClient.get(`rider_active:${rId}`);
            if (isActive) {
              availableRiders.push({ user_id: rId });
            }
          }
        } catch (err) {
          ...
          const pgRiders = await client.query(
