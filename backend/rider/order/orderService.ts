...
export const acceptOrder = async (riderId: number, orderIdRaw: string | number) => {
  ...
  if (activeCheck) {
    throw new AppError(
      'You can only have one active delivery at a time. Please complete your current delivery to accept new orders.',
      400
    );
  }
...
export const updateOrderStatus = async (riderId: number, orderIdRaw: string | number, status: import('@prisma/client').order_status) => {
  const orderId = parseInt(String(orderIdRaw), 10);
...
export const getOrderDetails = async (riderId: number, orderIdRaw: string | number) => {
  const orderId = parseInt(String(orderIdRaw), 10);
