...
export const signup = async (data: RiderSignupData) => {
  const {
    name,
    email,
    password,
  ...
  const newRider = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    // Insert into users
...
export const login = async (email: string, password: string) => {
  ...
  return {
    user: userDto,
    profile: (user as unknown as { rider_profiles: Record<string, unknown> }).rider_profiles || {},
  };
};
