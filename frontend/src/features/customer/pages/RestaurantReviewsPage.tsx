...
interface ExtendedReview extends Partial<Review> {
  user_name?: string;
}
      ...
      {reviews.length > 0 ? (
        <div className="space-y-6">
          {reviews.map((review, index) => {
            ...
            const ratingValue = review.rating ? parseFloat(String(review.rating)) : 0;
            ...
            return (
              ...
              >
                <div className="flex items-center mb-2">
                  ...
                  <div className="ml-4 flex items-center">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`h-5 w-5 ${i < ratingValue ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                      />
                    ))}
                ...
                <p className="text-sm text-gray-500">
                  Reviewed on: {review.created_at ? new Date(review.created_at).toLocaleDateString() : 'Recent'}
                </p>
              </div>
            );
          })}
