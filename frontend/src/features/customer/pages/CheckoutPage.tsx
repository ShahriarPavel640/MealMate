...
interface RestaurantOrderGroup {
...
const CheckoutPage: React.FC = () => {
  ...
  const ordersByRestaurant = useMemo(() => {
    return cartItems.reduce<Record<string, RestaurantOrderGroup>>((acc, item) => {
      const restaurant_id = String(item.restaurant_id || "unknown");
      ...
      acc[restaurant_id]!.subtotal += (item.price || 0) * item.quantity;
      return acc;
    }, {});
  }, [cartItems]);
  ...
  const handlePlaceOrder = async () => {
    ...
    const customerInfo = {
      ...
      address: typeof authUser.address === "string" ? authUser.address : "N/A",
