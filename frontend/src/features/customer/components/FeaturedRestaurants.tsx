...
const FeaturedRestaurants: React.FC<FeaturedRestaurantsProps> = ({ restaurants }) => {
  ...
  const handleFavoriteClick = (e: React.MouseEvent<HTMLButtonElement>, restaurantId: number) => {
    e.preventDefault();
    e.stopPropagation();
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {restaurants && restaurants.length > 0 ? (
            restaurants.slice(0, 9).map((restaurant) => {
              ...
              const displayImage = restaurant.image_url || restaurant.image || "";
              const displayRating = restaurant.average_rating ?? restaurant.avg_rating ?? "N/A";

              return (
                <Link
                  key={targetId}
                  to={`/restaurant/${targetId}`}
                  className="group"
                >
                  <div className="card bg-[#1a1a1a] border border-white/5 shadow-xl hover:shadow-2xl transition-all duration-300 h-full overflow-hidden hover:border-white/20 group-hover:-translate-y-2 rounded-3xl">
                    <figure className="relative overflow-hidden">
                      ...
                      <button 
                        onClick={(e) => handleFavoriteClick(e, targetId)}
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-300 bg-white/5 py-1.5 px-3 rounded-xl w-fit">
                            ...
                            <span>
                              {restaurant.distance 
                                ...
                                : `${15 + (targetId % 5) * 5}-${25 + (targetId % 5) * 5} min`}
                            </span>
                          </div>
                        </div>
