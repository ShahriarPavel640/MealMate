...
interface LiveTrackingModalProps {
  ...
  dropoffLocation?: LocationCoords | null;
}
...
export default function LiveTrackingModal({
  ...
  orderId,
  dropoffLocation,
}: LiveTrackingModalProps): React.JSX.Element {
  const [riderLocation, setRiderLocation] = useState<LocationCoords | null>(null);
  ...
  useEffect(() => {
    if (isOpen && orderId) {
      ...
      const handleLocationUpdate = (data: { latitude: string | number; longitude: string | number }) => {
  ...
  const getCenter = (): LocationCoords => {
