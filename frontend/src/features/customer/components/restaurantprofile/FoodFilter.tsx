...
import { MenuCategoryItem } from "./MenuCategories";
...
export const FoodFilter: React.FC<FoodFilterProps> = ({
  menuCategories,
  onFilterChange,
  maxPrice = 2000,
}) => {
  ...
  useEffect(() => {
    setFilters(prev => ({
      ...prev,
      priceRange: [prev.priceRange[0] ?? 0, maxPrice]
    }));
  }, [maxPrice]);

  const [isCategoryOpen, setIsCategoryOpen] = useState(false);

  const handleCategoryChange = (categoryId: number | string, checked: boolean) => {
    const newCategories = checked
      ? [...filters.selectedCategories, categoryId]
      : filters.selectedCategories.filter(id => id !== categoryId);
    
    const newFilters: FilterState = { ...filters, selectedCategories: newCategories };
    setFilters(newFilters);
    onFilterChange(newFilters);
  };

  const handlePriceChange = (value: number[]) => {
    const newFilters: FilterState = { ...filters, priceRange: value };
    setFilters(newFilters);
    onFilterChange(newFilters);
  };

  const handleSortByChange = (value: string) => {
    const newFilters: FilterState = { ...filters, sortBy: value };
    setFilters(newFilters);
    onFilterChange(newFilters);
  };

  const removeCategory = (categoryId: number | string) => {
                ...
                <div className="p-4 max-h-64 overflow-y-auto">
                  <div className="space-y-3">
                    {menuCategories.map((category, idx) => {
                      const catId = category.category_id ?? idx;
                      return (
