...
interface EarningsData {
  weekly: WeeklyStatItem[];
  monthly: MonthlyStatItem[];
  peakHours: PeakHourItem[];
}

interface RiderReviewItem {
  user_name: string;
  rating: number;
  comment: string;
  created_at: string;
...
const RiderEarningsDashboard: React.FC = () => {
  const [earningsData, setEarningsData] = useState<EarningsData>({ weekly: [], monthly: [], peakHours: [] });
  const [reviewsData, setReviewsData] = useState<ReviewsData>({ reviews: [], averageRating: null });
    ...
    const fetchData = async () => {
      try {
        ...
        setReviewsData({
          ...
          averageRating: reviewsRes.data.averageRating ?? null
        });
        setTotalReviewPages(reviewsRes.data.totalPages || 1);
        setLoading(false);
      } catch {
  ...
  const weeklyStats = earningsData.weekly.reduce((acc, day) => {
    acc.totalEarnings += parseFloat(String(day.earnings)) || 0;
    acc.totalOrders += parseInt(String(day.orders), 10) || 0;
    acc.totalHours += parseFloat(String(day.hours)) || 0;
    return acc;
