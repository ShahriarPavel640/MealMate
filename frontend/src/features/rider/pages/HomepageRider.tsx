...
const HomepageRider: React.FC = () => {
  const [dashboardData, setDashboardData] = useState<RiderDashboardData>({
    availableOrders: [],
    assignedOrders: [],
  });
  ...
  const availableOrdersRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const res = await axiosInstance.get<RiderDashboardData>(`/rider/stats/dashboard${query}`);
  ...
  useEffect(() => {
    if (authrider) {
      ...
      const handleNewDelivery = (newOrder: RiderOrder) => {
        toast.success(`New order #${newOrder.order_id} available!`);
        addNotification({
          type: "delivery_status",
          message: `A new delivery (#${newOrder.order_id}) is available from ${newOrder.restaurant_name || 'a restaurant'}.`,
      ...
      const handleDeliveryRemoved = ({ orderId }: { orderId: number }) => {
        setDashboardData((prevData) => {
          ...
          if (orderExists) {
            toast(`Order #${orderId} is no longer available.`);
            return {
              ...prevData,
  ...
  const handleUpdateOrderStatus = async (orderId: number, newStatus: string) => {
    try {
      ...
      toast.error(
        apiErr?.response?.data?.message || "Failed to update order status."
      );
    }
  };
  ...
  return (
    <RiderLayout onChatClick={() => { setSelectedOrderId(null); setIsChatModalOpen(true); }}>
      <ChatModal
        ...
        orderId={selectedOrderId !== null ? selectedOrderId : undefined}
      />
