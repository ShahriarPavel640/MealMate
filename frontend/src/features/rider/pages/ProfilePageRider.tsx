...
interface RiderProfileFormData {
  name: string;
  phone_number: string;
  vehicle_type: string;
...
const ProfilePageRider: React.FC = () => {
  ...
  const [formData, setFormData] = useState<RiderProfileFormData>({
    name: "",
    phone_number: "",
    vehicle_type: "",
    latitude: null,
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await axiosInstance.get<RiderProfileData>("/rider/profile");
        setProfile(res.data);
        setFormData({
          name: res.data.name,
    ...
    <RiderLayout>
      ...
      <LocationPickerModal
        ...
        initialLocation={
          formData.latitude && formData.longitude
            ? {
                lat: parseFloat(String(formData.latitude)),
                lng: parseFloat(String(formData.longitude)),
              }
            : null
        }
      />
