...
interface OrderCardProps {
  ...
  onRejectOrder: (orderId: number | string) => void;
}

function OrderCard({ order, onUpdateStatus, onRejectOrder }: OrderCardProps): React.JSX.Element {
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            <p className="text-xl font-bold text-purple-400">
              Tk {parseFloat(String(order.total_amount || 0)).toFixed(2)}
            </p>
            ...
            {order.items &&
              order.items.map((item, index) => (
                ...
                >
                  ...
                  <span>
                    Tk {(parseFloat(String(item.price || 0)) * item.quantity).toFixed(2)}
                  </span>
                </div>
              ))}
          </div>
...
function PaginationControls({ currentPage, totalPages, onPageChange }: PaginationControlsProps): React.JSX.Element | null {
...
function OrderManagement(): React.JSX.Element {
  ...
  const updateOrderStatus = useCallback(
    async (orderId: number | string, newStatus: string) => {
  ...
  const handleNewOrder = useCallback(
    (newOrder: OrderData) => {
      ...
      toast.success(
        ...
        } for Tk ${parseFloat(String(newOrder.total_amount || 0)).toFixed(2)}`,
  ...
  const handleOrderStatusUpdated = useCallback((updatedOrder: OrderData) => {
    setOrderList((prevOrders) =>
      prevOrders.map((order) =>
  ...
  const handleOrderAccepted = useCallback(({ orderId, riderProfile }: { orderId: number | string; riderProfile: { name: string; phone_number: string } }) => {
    setOrderList((prevOrders) =>
      prevOrders.map((order) =>
        order.order_id === orderId
          ? {
