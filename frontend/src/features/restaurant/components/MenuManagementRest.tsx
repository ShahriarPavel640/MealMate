...
function MenuManagementRest(): React.JSX.Element {
  const {
    initialMenuItems,
    get_menus,
    delete_menu_item,
  ...
  const handleSaveEdit = async (updatedItem: FormData) => {
  ...
  const avgPrice =
    menuItems.length > 0
      ? (
          menuItems.reduce((sum, item) => sum + parseFloat(String(item.price || 0)), 0) /
          menuItems.length
        ).toFixed(2)
      : "0.00";

  const handleToggleStatus = async (itemId: number | string) => {
    const item = menuItems.find((m) => m.menu_item_id === itemId);
    if (!item) return;
